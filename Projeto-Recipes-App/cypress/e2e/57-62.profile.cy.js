...
describe('Profile screen', () => {
  beforeEach(() => {
    cy.visit('http://localhost:3000/profile', {
      onBeforeLoad(win) {
        ...
        win.localStorage.setItem('drinksToken', '1');
  ...
  describe('60 - Redirecione a pessoa usuária que, ao clicar no botão de \"Done Recipes\", a rota deve mudar para a tela de receitas feitas', () => {
    ...
    it('Verifica a cobertura de 45% da tela de Perfil', () => {
      cy.task('getCoverage', getId()).its('Profile.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('Profile.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('Profile.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
    });
  });
  ...
  describe('62 - Redirecione a pessoa usuária que ao clicar no botão de \"Logout\", o `localStorage` deve ser limpo e a rota deve mudar para a tela de login', () => {
    it('Limpa todas as chaves da localStorage', () => {
      cy.window().then((win) => {
        ...
        expect(win.localStorage.getItem('drinksToken')).to.eq('1');
      ...
      cy.window().then((win) => {
        ...
        expect(win.localStorage.getItem('drinksToken')).to.be.null;
    ...
    it('Verifica a cobertura de 90% da tela de Perfil', () => {
      cy.task('getCoverage', getId()).its('Profile.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('Profile.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('Profile.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
    });
  });
});
